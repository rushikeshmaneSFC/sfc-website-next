...
export default function ScheduleModal({
  ...
  const validateCaptchaInput = () => {
    return validateCaptcha(userCaptchaInput);
  };
  ...
  const handleSubmit = async (values: typeof initialVals) => {
    // const isValid = await validateCaptchaInput();

    // if (!isValid) {
    //   return;
    // }

    const apiEndpoint = "https://stfox.com/api/services/enroll";
    const experiencingBreach = "https://stfox.com/api/breached";
    ...
    if (from && selectedOption === "experiencingBreach") {
      ...
      try {
        const response = await fetch(experiencingBreach, {
          method: "POST",
          headers: {
