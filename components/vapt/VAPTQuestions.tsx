"use client";

import ReusableCard from "@/components/atoms/ReusableCard";
import Wrapper from "@/components/atoms/Wrapper";
import { VaptFaqs } from "@/constants/vaptConstant";

function VAPTQuestions() {
  return (
    <Wrapper
      className=" bg-cover bg-center
lg:h-[473px] md:h-[473px] h-auto items-center justify-center flex"
    >
      <div className="  items-center flex flex-col relative">
        <h2 className="text-[20px] lg:text-[24px] font-bold text-center ">
          Frequently Asked Questions
        </h2>
        <div className="absolute lg:bg-gray-500 lg:border-b md:border-b md:bg-gray-500 w-full lg:top-[62%] md:top-[61%] top-[64%] "></div>
        <div className="grid grid-cols-1 md:grid-cols-2 pt-10  lg:px-10 md:px-5 ">
          {VaptFaqs.map((card, index) => (
            <ReusableCard
              key={index}
              title={card.title}
              description={card.description}
              className={
                index === 2
                  ? "md:col-span-2 flex justify-center mx-auto w-full lg:w-[420px] md:w-[330px] border-l  lg:h-[170px] md:h-[170px] h-[180px] md:pt-5     "
                  : "  border-l  h-[200px] py-2 "
              }
              boxShadowColor={card.boxShadowColor}
            />
          ))}
        </div>
      </div>
    </Wrapper>
  );
}

export default VAPTQuestions;
